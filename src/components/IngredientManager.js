            ...
            <div className="flex items-center justify-between mb-4 px-1">
                <div>
                    ...
                    <p className="text-xs text-gray-500">
                        {[...(userInventory.standard || []).filter(i => i.in_stock), ...(userInventory.custom || []).filter(i => i.in_stock)].length} 种原料
                    </p>
                </div>
                {!showCustomForm && (
                    <button
                        ...
                        <div className="flex flex-wrap gap-2 px-1" style={{ minWidth: 0 }}>
                            {userInventory.custom.map(item => (
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all duration-200 flex items-center gap-1.5 ${item.in_stock
