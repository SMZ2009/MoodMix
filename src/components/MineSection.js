...
const MineSection = ({ userInventory, onUpdateInventory, favorites, onSelectDrink, cardFeedback }) => {
    ...
    const allInventoryItems = useMemo(() => {
        return [
            ...userInventory.standard.map(i => ({ ...i, id: i.ing_id, name: i.name_cn, image: 'https://images.unsplash.com/photo-1556679343-c7306c1976bc?auto=format&fit=crop&w=100&q=80' })), // Placeholder image
            ...userInventory.custom.map(i => ({ ...i, id: `custom-${i.id}`, name: i.name_cn, image: 'https://images.unsplash.com/photo-1556679343-c7306c1976bc?auto=format&fit=crop&w=100&q=80' }))
        ];
    }, [userInventory]);
    ...
    return (
        <div className="flex flex-col bg-dreamy-gradient w-full h-[100vh] max-w-4xl mx-auto overflow-hidden">
            {/* 头部固定区域 */}
            <div className="flex-none bg-white/30 backdrop-blur-md z-10 border-b border-white/20">
                <div className="flex flex-col items-center pt-8 pb-4 px-6">
                    <div className="relative mb-2">
                        ...
                        <div className="absolute bottom-0 right-0 bg-white w-6 h-6 rounded-full flex items-center justify-center shadow-sm border border-gray-100">
                            <Edit3 size={12} className="text-gray-500" />
                        </div>
                    </div>
                    <h1 className="text-lg font-bold text-[#111813]">调饮爱好者</h1>
            ...
            <div className="flex-1 overflow-y-auto px-6 no-scrollbar" style={{ maxHeight: 'calc(100vh - 420px)' }}>
                {mineTab === 'favorites' && (
